...
def process_excel_files(file_list: list) -> None:
    ...
    for index, filepath in enumerate(file_list):
        ...
        sheet_index = 1
        for sheetname in sheet_tables:
            table_list = sheet_tables[sheetname]
            os.makedirs(f"output/ex_{index}", exist_ok=True)
            for count, markdown_text in enumerate(table_list, 1):
                filename = f"output/ex_{index}/sheet_{sheet_index}_table_{count}.md"
                with open(filename, 'w') as file:
                    file.write(markdown_text)
                print(f"Markdown Table from Sheet '{sheetname}' written to {filename}")
            sheet_index += 1
