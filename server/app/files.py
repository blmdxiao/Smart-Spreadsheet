...
async def parse_file_content_async(file_path: str, file_extension: str, file_md5: str, id_url_info: Dict[str, Any]) -> None:
    ...
    if file_loader_obj:
        if USE_LLAMA_PARSE:
            content = await file_loader_obj.get_content_by_llama_parse()
        else:
            content = await file_loader_obj.get_markdown_tables()
        ...
        if content:
            ...
            if USE_LLAMA_PARSE:
                await text_parser_obj.add_content(doc_id, content, url)
            else:
                await text_parser_obj.add_sheet_tables(doc_id, content, url)
        else:
