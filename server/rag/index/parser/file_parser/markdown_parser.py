...
class AsyncTextParser:
    ...
    async def add_sheet_tables(self, doc_id: int, sheet_tables: Dict[str, List[str]], url: str) -> None:
        if not sheet_tables:
            await self.update_doc_status(doc_id, LOCAL_FILE_PROCESS_FAILED)
            return

        begin_time = int(time.time())
        logger.info(f"[FILE_CONTENT] add_sheet_tables begin, doc_id: {doc_id}, begin_time: {begin_time}")
        await self.update_doc_status(doc_id, LOCAL_FILE_PARSING)
        ...
        for sheet_name in sheet_tables:
            table_list = sheet_tables[sheet_name]
            for content in table_list:
                ret = text_splitter_obj.split_text(content)
                chunk_text_vec.extend(ret)
        ...
        end_time = int(time.time())
        timecost = end_time - begin_time
        logger.warning(f"[FILE_CONTENT] add_sheet_tables end, end_time: {end_time}, timecost: {timecost}")
